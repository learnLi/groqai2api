...
func (t *TlsClient) handleHeaders(req *fhttp.Request, headers groq.Headers) {
	if headers == nil {
		return
	}
...
func (t *TlsClient) Request(method string, url string, headers groq.Headers, cookies []*http.Cookie, body io.Reader) (*http.Response, error) {
...
func (t *TlsClient) SetProxy(proxy string) {
	if proxy != "" {
		t.Client.SetProxy(proxy)
	}
}
